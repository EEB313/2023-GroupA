...
timeseries <- data.frame(when = seq.Date(min(colombiaAbundance$when), max(sanFranAbundance$when), by = "month"))
...
ggplot(regressionData, aes(x = squareKmBurnt, y = fourthRoot, color = incidentCounty)) + geom_line(aes(y=fit_IntSlope)) + geom_point() + geom_abline(aes(intercept=2.811, slope=2.312e-04))+ labs(x = "Area Burned by Fire (Square Kilometre)", y = "Number of Mushroom Observations^(1/4)") 
